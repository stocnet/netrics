...
#
# Ported from {manynet}'s data-raw/build_tutorial_articles.R so that the
# stocnet packages share one article-generation transform. Only the list of
# build_article() calls at the bottom differs.
#
...
#   chunks (learnr-only)
# - drops learnr context chunks (those with a `context=` option, e.g. the
#   `context="server-start"` language-detection chunk), which are meaningless
...
build_article <- function(src_path, out_path) {
  ...
  # learnr runtime chunks (e.g. context="server-start") make no sense in a
  # static article, so drop them like the other scaffolding
  chunk_opts <- vapply(chunks, function(x) x$opts, character(1))
  is_context <- grepl("context\\s*=", chunk_opts) & !is_quiz

  is_scaffold <- is_hint | is_solution_scaffold | is_setup_scaffold | is_context
...
build_article(
  "inst/tutorials/netrics1/centrality.Rmd",
...
build_article(
  "inst/tutorials/netrics3/position.Rmd",
  "vignettes/articles/position.Rmd"
)
build_article(
  "inst/tutorials/netrics4/topology.Rmd",
  "vignettes/articles/topology.Rmd"
)

# No data/ folder is copied: with `eval = FALSE` the data-loading calls are
# shown but never run, so the tutorial data files are not needed to render.
