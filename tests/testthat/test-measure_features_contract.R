...
test_that("measures on several scales report which one they are on", {
  ...
  expect_equal(attr(net_by_core(g, method = "ident"), "measure"),
               "core-periphery distance")
  expect_equal(attr(net_by_core(g, method = "ident"), "range"), c(0, Inf))
  # Sigma is a ratio of ratios with no upper bound; omega and SWI are bounded.
  expect_equal(attr(net_by_smallworld(g, method = "sigma", times = 20), "range"),
               c(0, Inf))
  ...
  expect_equal(attr(net_by_smallworld(g, method = "SWI", times = 20), "variant"),
               "SWI")
  # A variant is orthogonal to a normalisation: SWI is both.
  expect_equal(attr(net_by_smallworld(g, method = "SWI", times = 20),
                    "normalization"), "normalized")
  expect_equal(attr(net_by_core(g, method = "ident"), "variant"), "ident")
