...
for(fn in names(net_meas)) {
  for (ob in names(data_objs)) { 
    test_that(paste(fn, "works on", ob), {
      skip_if(grepl("congruency|correlation|degree|core|change|infection|reach|immunity|recovery|reproduction|richclub|stability|balance|strength|waves|transmiss|spatial", fn))
