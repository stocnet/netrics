...
```{r brokerrole-qa, echo=FALSE, purl = FALSE}
question("A lawyer receives advice from a colleague in another office and passes it on to a colleague in their *own* office. Which Gould-Fernandez brokerage role is this?",
         answer("Gatekeeper",
                correct = TRUE,
                message = paste(learnr::random_praise(), "The broker and the target share a group while the source is an outsider, so the broker governs what gets *in* to their own group.")),
         answer("Representative",
                message = "Not quite -- a representative sends its *own* group's resource *out* to an outsider; here the flow runs the other way, from an outsider *in* to the broker's group."),
         answer("Coordinator",
                message = "Not quite -- a coordinator brokers between two members of its *own* group; here the source sits in a different office."),
         answer("Liaison",
                message = "Not quite -- a liaison brokers between two groups that are *both* different from its own; here the broker and the target share an office."),
         random_answer_order = TRUE,
         allow_retry = TRUE
)
```
...
```{r broker-setup}
lawfirm_advice <- to_uniplex(ison_lawfirm, "advice")
```

**Run the code to count each lawyer's brokerage roles in the advice network,
grouped by office.**

```{r nodebrokerage, exercise = TRUE, exercise.setup = "broker-setup"}
...
```{r netbrokerage, exercise = TRUE, exercise.setup = "broker-setup"}
...
question("Structural equivalence means identifying classes of nodes with...",
         answer("same/similar tie partners.",
                ...
                message = paste0(learnr::random_praise(),
                                 ' <br><br> <img src="https://i.giphy.com/media/v1.Y2lkPTc5MGI3NjExNGhvaTZ2Z3NxOW0yeTJ6ZjVoZ2ZqM2NqMjlxbXp1YnBmb2VtdTBldiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/XcFqv8EzBiAlzeEID9/giphy.gif" alt="gif of rick multiplying"/>')),
...
question("In a blockmodel plot sorted by class, what does an empty (white) off-diagonal block between class A (rows) and class B (columns) tell you?",
         answer("Members of class A rarely or never send ties to members of class B.",
                correct = TRUE,
                message = paste0(learnr::random_praise(),
                                 ' <br><br> <img src="https://i.giphy.com/media/v1.Y2lkPTc5MGI3NjExeHVzdGlhOGkyOWg2Z3BnMmE2eDgxMnlhZHhwaWx0bGZ3dTNmZnYyaSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/dsKkAGVnds7aXQKBI7/giphy.gif" alt="gif of mortys in a block parade"/>')),
         answer("Classes A and B are the same class.",
                message = 'Not quite -- an empty block is about the *absence of ties* from one class to another, not about the classes being identical. Identical classes wouldn\'t be drawn as two separate blocks. <br><br> <img src="https://i.giphy.com/media/v1.Y2lkPTc5MGI3NjExdmRlczhzaDJzaTBma3Q0c3pydDJjZTFkY20yMmhzbW13YmU3cnl6ZSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9dg/QIEol1DjEwSOc9PvLu/giphy.gif" alt="gif of mortys learning about roles"/>'),
